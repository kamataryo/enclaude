#!/usr/bin/env bash
# enclaudé worktree の後片付けを検証する。docker-compose はダミーに差し替えるので Docker は不要。
set -euo pipefail
...
wt() { (cd "$repo" && FAKE_ACTION="${2:-}" "$here/bin/enclaudé" worktree "$1" >/dev/null); }

echo "変更なし → worktree もブランチも消える"
wt clean
check "worktree が消えている"  '[ ! -d "$tmp/wt_clean_repo" ]'
check "ブランチも消えている"    '! git -C "$repo" rev-parse --verify -q wt/enclaude/clean >/dev/null'
...
check "ブランチが残っている"    'git -C "$repo" rev-parse --verify -q wt/enclaude/done >/dev/null'

echo "未コミットあり → worktree ごと残る"
wt dirty 'echo x >> a.txt'
check "worktree が残っている"  '[ -d "$tmp/wt_dirty_repo" ]'
check "ブランチが残っている"    'git -C "$repo" rev-parse --verify -q wt/enclaude/dirty >/dev/null'
