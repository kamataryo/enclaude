...
シェルを開き直してからイメージを用意してください。

### 設定を上書きする（任意）

claude に渡す設定はリポジトリの `settings.json` に入っています。ホストの `~/.claude/settings.json` はコンテナからは見えないので、持ち込みたい設定がある場合は `settings.override.json`（git 管理外）に書いてください。
