...
ロックの更新はコンテナの中で行うので、ホストに Node.js や pnpm は必要ありません。
まずロックだけを更新して上がる先のバージョンを表示し、再ビルドしてよいか確認します。`n` と答えるとロックも元に戻すので、ロックとイメージが食い違ったまま残ることはありません。更新がなければ再ビルドはスキップします。

```
claude-code 2.1.220 -> 2.1.251 に更新します。イメージを再ビルドしますか? [Y/n]
```
